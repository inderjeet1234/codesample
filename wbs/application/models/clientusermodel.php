...
class Clientusermodel extends CI_Model {
    ...
    public function getusers($startlimit=0,$limit=0) {
        ...
        {
            $query = $this->db->query("select * from users where company_type=2 and username  LIKE '".$this->input->post('searchfil')."%' or company_type=2 and name  LIKE '".$this->input->post('searchfil')."%'");
        }
