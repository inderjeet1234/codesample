...
class Clientuser extends Base_Controller {
    ...
    public function create()
    {
        ...
        $this->load->model('clientusermodel','',true);
        if($this->uri->segment(4))
        {
            $this->load->model('clientusermodel','',true);
            $data['val']=$this->clientusermodel->getmainuserdetails();
        }
        else
        {
            $val=array(
        ...
        $data['select']=$this->clientusermodel->getcompanies();
        $this->load->view('common/header');
        $this->load->view('common/left');
        $this->load->view('common/main');
        $this->load->view('clientuser/create',$data);
    ...
    public function users()
    {
        ...
        {
            $data['total_rows'] = $this->db->query("select * from users where company_type=2 and username  LIKE '".$this->input->post('searchfil')."%' or company_type=2 and name  LIKE '".$this->input->post('searchfil')."%'")->num_rows();
        ...
        else
        {
            ...
            $data['total_rows'] = $this->db->get_where('users',array('company_type'=>2))-> num_rows();
            ...
            $this->load->model('clientusermodel','',true);
            $data['data']=$this->clientusermodel->getusers($data['startlimit'],$data['per_page']);

        }

        $data['companiesfilters']=$this->clientusermodel->getcompanies();

        $this->load->view('common/header');
        $this->load->view('common/left');
        $this->load->view('common/main');
        $this->load->view('clientuser/listusers',$data);
        $this->load->view('common/footer');
